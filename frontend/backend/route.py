...
try:
    ...
    print(f"Failed to import FastAPI/Mangum: {e}")
    raise
...
try:
    ...
    print("TrustCode AI Audit API initialized successfully")
except Exception as e:
    print(f"Failed to initialize TrustCode AI Audit API: {e}")
    print(traceback.format_exc())
    # Re-raise to fail fast during development
    raise
...
async def audit_code(request: Request):
    ...
    try:
        ...
        print(f"Audit failed: {traceback.format_exc()}")
...
async def audit_zip(file):
    ...
    for file_name in file_list:
        ...
        try:
            ...
            print(f"Error processing file {file_name}: {traceback.format_exc()}")
            file_results.append({
                'file': file_name,
                'error': str(e),
                'TrustScore': 0,
...
# Only create handler if app was successfully initialized
if app is not None:
    handler = Mangum(app)
else:
    # This should not happen in production, but prevents import errors
    handler = None
